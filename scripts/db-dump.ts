...
async function main() {
  ...
  const [
    ...
    prisma.user.findMany({
      select: {
        ...
        examScheduledAt: true, formationId: true, gender: true, portfolioEnabled: true,
        portfolioSlug: true, blockedReason: true, lastBlockedAt: true,
        resetPasswordRequired: true, status: true, portfolioStatus: true,
        // password excluded
      },
    }),
    ...
    prisma.portfolioMission.findMany(),
    prisma.userPortfolioMission.findMany(),
    prisma.portfolioProof.findMany(),
  ]);
  ...
  const dump = {
    ...
    tables: {
      ...
      settings, reports, waitlist, exams, examParts, questions, questionOptions,
      resources, examSessions, compositionScans, internshipRequests,
      correctionRequests, securityLogs, auditLogs, chatMessages, notifications,
      contacts, reclamations, portfolioMissions, userPortfolioMissions, portfolioProofs,
    },
    counts: {
      ...
      reports: reports.length, waitlist: waitlist.length, exams: exams.length,
      examParts: examParts.length, questions: questions.length,
      questionOptions: questionOptions.length, resources: resources.length,
      ...
      chatMessages: chatMessages.length, notifications: notifications.length,
      contacts: contacts.length, reclamations: reclamations.length,
      portfolioMissions: portfolioMissions.length,
      userPortfolioMissions: userPortfolioMissions.length,
      portfolioProofs: portfolioProofs.length,
    },
  };
