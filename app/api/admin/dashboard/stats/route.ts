...
export async function GET(request: Request) {
  try {
    ...
    })) as Array<{ issuedAt: Date; type: string; status: string }>;
    ...
    const monthlyData: Record<
      string,
      { registrations: number; attestations: number; validations: number }
    > = {};
    ...
    attestations.forEach((a) => {
      const label = new Date(a.issuedAt).toLocaleDateString("fr-FR", {
        month: "short",
      });
    ...
    users.forEach((u) => {
      const label = new Date(u.createdAt).toLocaleDateString("fr-FR", {
        month: "short",
      });
      if (monthlyData[label]) {
