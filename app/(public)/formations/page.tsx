...
export default function PublicFormationsPage() {
  ...
  useEffect(() => {
    async function fetchFormations() {
      try {
        const response = await fetch('/api/public/formations');
  ...
  const categories = ["Toutes", ...Array.from(new Set(formations.map(f => f.category)))];
  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] selection:bg-emerald-100 selection:text-emerald-900 pb-24 overflow-x-hidden">
      {/* --- PREMIUM GLOW DECOR --- */}
          ...
          >
            ...
            <div className="w-full max-w-4xl pt-8 px-2 md:px-0">
              ...
              >
                ...
                <div className="relative flex flex-col md:flex-row items-stretch md:items-center bg-white/90 backdrop-blur-3xl rounded-[2rem] border border-white shadow-2xl shadow-slate-200/50 p-2 overflow-hidden">
              ...
              {/* Active Pills (Optional but premium) */}
      ...
      {/* --- FORMATIONS GRID --- */}
      <section className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 mt-12">

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <Card key={i} className="p-8 space-y-6 rounded-[2.5rem] bg-white/50 border-white shadow-2xl">
                    ...
                    <div className="relative z-10 flex flex-col h-full">
                      ...
                      <div className="mt-auto">
                        <Link href={`/auth?register=true&formationId=${f.id}`} className="block">
                          <Button className="w-full h-14 md:h-16 rounded-2xl bg-slate-900 text-white font-black hover:bg-emerald-600 hover:shadow-2xl hover:shadow-emerald-500/20 transition-all duration-500 group/btn active:scale-[0.98] text-[10px] md:text-xs uppercase tracking-wider">
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 mt-32 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             <div className="lg:col-span-2 row-span-2 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[3rem] p-10 md:p-12 text-white flex flex-col justify-end min-h-[350px] shadow-2xl shadow-emerald-900/20 group">
